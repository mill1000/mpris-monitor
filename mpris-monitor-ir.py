...
class MprisDbusMonitor(threading.Thread):
    ...
    def run(self):
        ...
        bus.subscribe(object="/org/mpris/MediaPlayer2", iface="org.freedesktop.DBus.Properties",
                      signal="PropertiesChanged", signal_fired=self.properties_changed)

        # Subscribe to name changes
        bus.subscribe(object="/org/freedesktop/DBus", iface="org.freedesktop.DBus", signal="NameOwnerChanged",
                      arg0="org.mpris.MediaPlayer2", flags=Gio.DBusSignalFlags.MATCH_ARG0_NAMESPACE, signal_fired=self.name_owner_changed)
    ...
    def properties_changed(self, sender, object, iface, signal, params):
        ...
        logger.debug("'{0}' '{1}' '{2}' '{3}' = '{4}'".format(
            sender, object, iface, signal, params))
...
class SystemController():
    ...
    def update(self, sender, state):
        ...
        if state == "Playing":
            ...
            logger.debug("Starting short ({0} s) shutdown timer.".format(
                self.short_timeout))
...
def main(args):
    ...
    def power_on():
        subprocess.run(["ir-ctl", "-d", "/dev/lirc1",
                       "--scancode", "necx:0x404003"])

    def power_off():
        subprocess.run(["ir-ctl", "-d", "/dev/lirc1",
                       "--scancode", "necx:0x404000"])
